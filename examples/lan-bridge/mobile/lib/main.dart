...
lebih berguna.', style: const TextStyle(fontSize: 36, height: .98, fontWeight: FontWeight.w800, letterSpacing: -1.8)), const SizedBox(height: 13), Text(connection.connected ? 'Pilih mode yang ingin digunakan. Semua mode memakai sesi LAN yang sama.' : 'Hubungkan ke Halo Deck Desktop Hub di jaringan WiFi yang sama—cukup satu scan QR.', style: TextStyle(color: Colors.white.withOpacity(.64), height: 1.5, fontSize: 14)), const SizedBox(height: 23), if (!connection.connected) _pairCard() else _connectedCard(), const SizedBox(height: 18), if (services.isNotEmpty) _discoveryList() else if (!connection.connected) _discoveryButton()]));
  ...
  Widget _scannerView() => Stack(children: [MobileScanner(controller: scanner, onDetect: _scanResult), Positioned.fill(child: IgnorePointer(child: DecoratedBox(decoration: BoxDecoration(color: Colors.black.withOpacity(.2))))), Center(child: Container(width: 270, height: 270, decoration: BoxDecoration(border: Border.all(color: apricot, width: 3), borderRadius: BorderRadius.circular(22)))), Positioned(top: 28, left: 20, right: 20, child: Row(children: [IconButton(onPressed: () async { await scanner.stop(); setState(() => scanning = false); }, icon: const Icon(Icons.close)), const Text('Scan QR Desktop Hub', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 17))])), const Positioned(bottom: 45, left: 28, right: 28, child: Text('Arahkan kamera ke QR pada layar Desktop Hub. Pairing berlangsung otomatis.', textAlign: TextAlign.center, style: TextStyle(color: Colors.white, fontSize: 13, height: 1.4))]);

  Widget _modeBar() => NavigationBar(backgroundColor: const Color(0xFF132125), selectedIndex: mode.index, onDestinationSelected: (index) => _selectMode(PocketMode.values[index]), destinations: const [NavigationDestination(icon: Icon(Icons.dashboard_outlined), selectedIcon: Icon(Icons.dashboard), label: 'Home'), NavigationDestination(icon: Icon(Icons.monitor_outlined), selectedIcon: Icon(Icons.monitor), label: 'Layar'), NavigationDestination(icon: Icon(Icons.touch_app_outlined), selectedIcon: Icon(Icons.touch_app), label: 'Trackpad'), NavigationDestination(icon: Icon(Icons.access_time), selectedIcon: Icon(Icons.schedule), label: 'Jam')]);
